package vidupe.filter.constants;

public class VideoEntityProperties {
    public static final String EXISTS_IN_DRIVE = "exists-in-drive";
    public static final String VIDEO_NAME = "video-name";
    public static final String DURATION = "duration";
    public static final String VIDEO_LAST_MODIFIED = "video-last-modified";
    public static final String DEDUPE_PROCESS = "dedupe-processed";
    public static final String LAST_PROCESSED = "last-processed";
    public static final String PHASHGEN_PROCESSED = "phashgen-processed";
    public static final String NUM_KEYFRAMES = "number-of-keyframes";
    public static final String VIDEO_SIZE = "video-size";
}
